...
if len(sys.argv) == 1:
    genesis_command = 'mpirun -np 32 atdyn'
elif len(sys.argv) == 2:
...
if (genesis_command == "-h") or (genesis_command == "--help"):
    ...
    # run atdyn tests
    $ ./test_remd.py "mpirun -np 8 /path/to/genesis/bin/atdyn"

    # run spdyn tests
    $ ./test_remd.py "mpirun -np 8 /path/to/genesis/bin/spdyn"

    # run with specfic tolerance value
    $ ./test_remd.py "mpirun -np 8 /path/to/genesis/bin/atdyn" 0.1
...
if (is_fugaku):
    ...
    if not int(genesis_mpi_number)%4 == 0:
        print("Error: Number of MPI processes %d should be multiplier of 4" % int(genesis_mpi_number))
        sys.exit(3)
...
if (is_atdyn or is_spdyn):
    print("=======================================================================")
    print(" Regression tests for REMD")
    print("=======================================================================")
