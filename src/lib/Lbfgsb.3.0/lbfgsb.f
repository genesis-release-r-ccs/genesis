...
c_YA            open (8, file = 'iterate.dat', status = 'unknown')
c_YA_Bgn
c            open (98, file = 'iterate.dat', status = 'unknown')
            write(*, '("Rank: ",i0)')  rank
            write(a,*) rank
            open (98, file = 'iterate.dat'//trim(adjustl(a)), 
     +            status = 'unknown')
c_YA_End
         endif            
...
c@ky      if ((fold - f) .le. tol*ddum) then
c@ky     change from (fold - f) to |fold - f| 
...
c                             the truncated Newton step has been used.
 ...
 2001 format
     +  (/,'At iterate',i5,4x,'f= ',1p,d12.5,4x,'|proj g|= ',1p,d12.5)
 3001 format(2(1x,i4),2(1x,i5),2x,a3,1x,i4,1p,2(2x,d8.1),1p,2(1x,f10.3))
