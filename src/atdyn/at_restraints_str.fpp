  ...
  subroutine alloc_restraints(restraints, variable, var_size, var_size2)
    ...
    case (RestraintsFunc)
      ...
      if (allocated(restraints%function)) then
        if (size(restraints%function(:)) == var_size) return
    ...
    case (RestraintsGroup)
      ...
      if (allocated(restraints%group)) then
        if (size(restraints%group(:)) == var_size) return
        deallocate(restraints%group, &
                   stat = dealloc_stat)
      end if

      allocate(restraints%group(var_size), &
               stat = alloc_stat)
    ...
    case (RestraintsList)
      ...
      if (allocated(restraints%atomlist)) then
        if (size(restraints%num_atoms(:)) == var_size) return
...
end module at_restraints_str_mod
