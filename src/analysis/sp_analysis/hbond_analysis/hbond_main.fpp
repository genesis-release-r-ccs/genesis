...
program hbond_main
  ...
  type(s_hbond_ctrl_data) :: hbond_ctrl_data
  type(s_hbond_option)    :: hbond_option
  ...
  ! show usage
  !
  call hbond_usage(ctrl_filename)
  ...
  !
  if (main_rank) then
    write(MsgOut,'(A)') '[STEP1] Read Control Parameters for H-bond analysis'
    write(MsgOut,'(A)') ' '
  end if

  call control(ctrl_filename, ctrl_data)
  call hbond_control(ctrl_filename, hbond_ctrl_data)
  ...
  call hbond_setup_option(hbond_ctrl_data%hbond_opt_info, hbond_option)
  ...
  call run_hbond(molecule, trj_list, output, option,  &
  ...
  call dealloc_hbond_option(hbond_option)
