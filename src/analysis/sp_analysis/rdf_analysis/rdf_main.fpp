...
program rdf_main
  ...
  type(s_rdf_ctrl_data)  :: rdf_ctrl_data
  type(s_rdf_option)     :: rdf_option
  ...
  ! show usage
  !
  call rdf_usage(ctrl_filename)

  ! [Step1] Read control file
  !
  if (main_rank) then
    write(MsgOut,'(A)') '[STEP1] Read Control Parameters for RDF analysis'
    write(MsgOut,'(A)') ' '
  end if

  call control(ctrl_filename, ctrl_data)
  call rdf_control(ctrl_filename, rdf_ctrl_data)
  ...
  call rdf_setup_option(rdf_ctrl_data%rdf_opt_info, rdf_option)
  ...
  call run_rdf(molecule, trj_list, output, option, rdf_option, &
  ...
  call dealloc_rdf_option(rdf_option)
