  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./distmat_analysis INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./distmat_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in distmat_analysis'
        write(MsgOut,'(A)') ' '
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                 DISTMAT_ANALYSIS                 *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*             distance matrix analyzer             *'
      write(MsgOut,'(A)')'*                                                  *'
