...
contains
  ...
  !
  !  Subroutine    usage
  !> @brief        show usage of qval_residcg_analysis
  !! @authors      NT
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./qval_residcg_analysis INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./qval_residcg_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in qval_residcg_analysis'
        write(MsgOut,'(A)') ' '
         
        call show_ctrl_input ('grotop,grocrd')
        call show_ctrl_output('qnt')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*               QVAL_RESIDCG_ANALYSIS              *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*      Analyze the fraction of native contacts     *'
      write(MsgOut,'(A)')'*                                                  *'
