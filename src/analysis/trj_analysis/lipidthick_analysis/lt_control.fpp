!--------1---------2---------3---------4---------5---------6---------7---------8
!
!  Module   lt_control_mod
!> @brief   read parameters and data for lipid bilayer thickness analysis
...
contains
  ...
  !
  !  Subroutine    usage
  !> @brief        show usage of lipidthick_analysis
  !! @authors      NT
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./lipidthick_analysis INPCNV'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./lipidthick_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in lipidthick_analysis'
        write(MsgOut,'(A)') ' '
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                LipidThick_ANALYSIS               *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*        analysis of lipid bilyaer thickness       *'
      write(MsgOut,'(A)')'*                                                  *'
  ...
  subroutine control(filename, ctrl_data)
    ...
    if (handle == 0) &
      call error_msg('Control_LipidThick_analysis> File IO Error')
