...
contains
  ...
  !
  !  Subroutine    usage
  !> @brief        show usage of fret_analysis
  !! @authors      NT
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./fret_analysis INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./fret_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in fret_analysis'
        write(MsgOut,'(A)') ' '
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                   FRET_ANALYSIS                  *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*          calculate the FRET Efficiency           *'
      write(MsgOut,'(A)')'*                                                  *'
