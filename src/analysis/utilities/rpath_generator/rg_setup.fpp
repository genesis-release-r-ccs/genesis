...
!
!  Module   rg_setup_mod
!> @brief   setup variables and structures in rpath_generator
...
contains
  ...
  subroutine setup(ctrl_data, molecule, input, output, fitting, option)
    ...
    ! define molecules
    !
    if (ctrl_data%inp_info%pdbfile /= '' .or. &
        ctrl_data%inp_info%fitfile /= '' .or. &
        ctrl_data%inp_info%ambcrdfile  /= '' .or. &
        ctrl_data%inp_info%grocrdfile  /= '') then

      if (ctrl_data%inp_info%fitfile == '') then
        ...
        call define_molecules(molecule, pdb=fit,       &
                                        psf=psf,       &
                                        prmtop=prmtop, &
                                        ambcrd=ambcrd, &
                                        grotop=grotop, &
    ...
    call setup_option(ctrl_data%opt_info, ctrl_data%sel_info, &
                      molecule, input, option)
    option%fitting_atom = ctrl_data%fit_info%fitting_atom

    return

  end subroutine setup

end module rg_setup_mod
