...
! 
!> Program  qg_main
!! @brief   Extract snapshots from MD trajectory format
...
program qg_main
  ...
  !
  write(MsgOut,'(A)') '[STEP1] Read Control Parameters for Generating QM/MM Inputs'
  write(MsgOut,'(A)') ' '
  ...
  ! [Step3] Take snapshots from trajectory files
  !
  write(MsgOut,'(A)') '[STEP3] Take Snapshots from Trajectory File(s)'
  write(MsgOut,'(A)') ' '

  call extframe(molecule,   &
                ref,        &
                trj_list,   &
                trajectory, &
                fitting,    &
