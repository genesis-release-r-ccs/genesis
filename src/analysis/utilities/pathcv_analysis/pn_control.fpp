...
contains
  ...
  !
  !  Subroutine    usage
  !> @brief        show usage of pathcv_analysis
  !! @authors      NT
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./pathcv_analysis INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./pathcv_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in pathcv_analysis'
        write(MsgOut,'(A)') ' '
         
        call show_ctrl_input ('path,CV')
        call show_ctrl_output('PATHCV')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                 PATHCV_ANALYSIS                  *'
      write(MsgOut,'(A)')'*                                                  *'
