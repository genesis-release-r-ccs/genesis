  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./prjcrd_analysis INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./prjcrd_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in prjcrd_analysis'
        write(MsgOut,'(A)') ' '

        call show_ctrl_input ('psf,ref,prmtop,ambcrd,grotop,grocrd,'//&
                              'pdb_ave,pdb_aft,val,vec')
        call show_ctrl_output('prj')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                 PRJCRD_ANALYSIS                  *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*         project each snapshot to PC-axis         *'
      write(MsgOut,'(A)')'*                                                  *'
