  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./dssp_interface INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./dssp_interface -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in dssp_interface'
        write(MsgOut,'(A)') ' '
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                 DSSP_INTERFACE                   *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                 dssp interface                   *'
      write(MsgOut,'(A)')'*                                                  *'
