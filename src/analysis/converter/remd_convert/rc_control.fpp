!--------1---------2---------3---------4---------5---------6---------7---------8
!
!  Module   rc_control_mod
!> @brief   read parameters and data for REMD trajectory converter
...
contains
  ...
  !
  !  Subroutine    usage
  !> @brief        show usage of remd_convert
  !! @authors      NT
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./remd_convert INPCNV'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./remd_convert -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in remd_convert'
        write(MsgOut,'(A)') ' '

        call show_ctrl_input ('psf,ref,prmtop,ambcrd,grotop,grocrd,REM,DCD,LOG')
        call show_ctrl_output('pdb,TRJ,LOG')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                   REMD_CONVERT                   *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*          Converter of REMD trajectory files      *'
      write(MsgOut,'(A)')'*                                                  *'
  ...
  subroutine control(filename, ctrl_data)
    ...
    if (handle == 0) &
      call error_msg('Control_Rmdcnv> File IO Error')
