!--------1---------2---------3---------4---------5---------6---------7---------8
!
!  Module   sp_energy_pme_noopt_2dalltoall_mod
...
module sp_energy_pme_noopt_2dalltoall_mod
  ...
  real(dp), public, save :: u_self_no2   ! Ewald self energy
  real(wp),         save :: inv_volume
  ...
  real(wp),         save, allocatable :: theta_lj(:,:,:) 
  ...
  ! subroutines
  public  :: setup_pme_noopt_2dalltoall
  public  :: dealloc_pme_noopt_2dalltoall
  public  :: pme_pre_noopt_2dalltoall
  public  :: pme_pre_noopt_2dalltoall_lj
  public  :: pme_recip_noopt_2dalltoall
  public  :: pme_recip_noopt_2dalltoall_lj

contains

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    setup_pme_noopt_2dalltoall
  ...
  subroutine setup_pme_noopt_2dalltoall(domain, boundary, enefunc)
    ...
    if (reciprocal_calc) then
     ...
     ! Check grid point in x direction
      ...
      if (my_x_rank == 0) then
        ...
        x_local1 = nlocalx1 - 1
        x_start1 = my_x_rank*nlocalx/2 + 2
    ...
    if (reciprocal_calc) then
      ...
      if (enefunc%vdw == VDWPME) then
        allocate(gx(nix+1), gy(niy), gz(ngrid(3)),      &
                 ...
                 ldf_work(nlocalx*nlocaly*nlocalz,1),   &
                 ftqdf(ngrid(3)*(nix+1)*niy),           &
                 ftqdf_work(ngrid(3)*(nix+1)*niy,1),    &
                 ftldf(ngrid(3)*(nix+1)*niy),           &
                 ftldf_work(ngrid(3)*(nix+1)*niy,1),    &
                 stat = alloc_stat)
      else
        allocate(gx(nix+1), gy(niy), gz(ngrid(3)),      &
                 vir_fact(ngrid(3),niy,nix+1),          &
  ...
  end subroutine setup_pme_noopt_2dalltoall

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    dealloc_pme_noopt_2dalltoall
  ...
  subroutine dealloc_pme_noopt_2dalltoall(enefunc)
    ...
    if (enefunc%vdw == VDWPME) then
      deallocate(gx, gy, gz, vir_fact, vir_fact_lj, vi,        &
                 theta, theta_lj, ftqdf_work2, ftldf_work2,    &
                 qdf, qdf_real, qdf_work, ftqdf, ftqdf_work,   &
                 ldf, ldf_real, ldf_work, ftldf, ftldf_work,   &
                 stat = dealloc_stat)
    else
      deallocate(gx, gy, gz, vir_fact, vi, theta, ftqdf_work2, &
                 qdf, qdf_real, qdf_work, ftqdf, ftqdf_work,   &
                 stat = dealloc_stat)
    end if
    if (dealloc_stat /= 0) call error_msg_dealloc
  ...
  end subroutine dealloc_pme_noopt_2dalltoall

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_pre_noopt_2dalltoall
  ...
  subroutine pme_pre_noopt_2dalltoall(domain, boundary)
    ...
    do i = 1, domain%num_cell_local
      do ix = 1, domain%num_atom(i)
        u_self_no2 = u_self_no2 + domain%charge(ix,i)*domain%charge(ix,i)
      end do
    end do

    u_self_no2 = - u_self_no2 * el_fact * alpha/sqrt(PI)

    return

  end subroutine pme_pre_noopt_2dalltoall

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_pre_noopt_2dalltoall_lj
  !> @brief        Prepare functions for PME calculation
  !! @authors      JJ, NT
  !! @param[in]    domain   : domain information
  !! @param[in]    boundary : boundary information
  ...
  subroutine pme_pre_noopt_2dalltoall_lj(domain, boundary)
  ...
  end subroutine pme_pre_noopt_2dalltoall_lj

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_recip_noopt_2dalltoall
  ...
  subroutine pme_recip_noopt_2dalltoall(domain, force, virial, eelec)
...
!ocl nosimd
    ...
    nix1 = nix + 1
    niy = ngrid(2)/nprocz
    call fft3d_2d_alltoall(qdf_work, ftqdf, ftqdf, ftqdf, ftqdf_work,      &
    ...
    do ix = 1, nix
      do iy = 1, niy
        ...
        vzz = 0.0_wp
        do iz = 1, ngrid(3)
          tqq = real(ftqdf_work2(iz,iy,ix),wp)**2 + imag(ftqdf_work2(iz,iy,ix))**2
    ...
    if(my_x_rank == 0 .and. my_y_rank == 0) then
      ...
      do iy = 1, niy
        ...
        vzz = 0.0_wp
        do iz = 1, ngrid(3)
          tqq = real(ftqdf_work2(iz,iy,1),wp)**2 + imag(ftqdf_work2(iz,iy,1))**2
        ...
        do iz = 1, ngrid(3)
          tqq = real(ftqdf_work2(iz,iy,nix1),wp)**2 + imag(ftqdf_work2(iz,iy,nix1))**2
    ...
    call bfft3d_2d_alltoall(qdf_work, qdf_real, ftqdf, ftqdf, ftqdf,       &
                            ...
                            work2, nlocalx, nlocaly, nlocalz,              &
                            ngrid(1), ngrid(2), ngrid(3),                  &
                            nprocx, nprocy, nprocz, nix, nix1, niy, niz,   &
...
!ocl nosimd
  ...
  end subroutine pme_recip_noopt_2dalltoall

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_recip_noopt_2dalltoall_lj
  !> @brief        Calculate PME reciprocal part with domain decomposition
  !! @authors      JJ, NT
  !! @param[in]    domain  : domain information
  !! @param[in]    enefunc : potential energy function information
  ...
  subroutine pme_recip_noopt_2dalltoall_lj(domain, enefunc, force, virial, &
                                           eelec, evdw)
    ...
    !$omp         work1, work2, work3, work4, elec_temp, evdw_temp,            &
    !$omp         force_local, ixyz, vx_tmp, vy_tmp, vz_tmp, nix, nix1, niy,   &
    !$omp         niz, bsc_tmp, bscd_tmp, kk, iprocx, iprocy, qtmp, grid,      &
    !$omp         half_grid, is, start, end, nizx, nizy, iatmcls, ltmp,        &
    !$omp         efft_real, efft_imag, vfft_real, vfft_imag, force_local_lj)
    !
#ifdef OMP
    id = omp_get_thread_num()
#else
...
!ocl nosimd
  ...
  end subroutine pme_recip_noopt_2dalltoall_lj

end module sp_energy_pme_noopt_2dalltoall_mod
