!--------1---------2---------3---------4---------5---------6---------7---------8
!
!  Module   sp_energy_pme_opt_slab_mod
...
module sp_energy_pme_opt_slab_mod
  ...
  real(dp), public, save :: u_self_os    ! Ewald self energy
  ...
  ! subroutines
  public  :: setup_pme_opt_slab
  public  :: dealloc_pme_opt_slab
  public  :: pme_pre_opt_slab
  public  :: pme_recip_opt_slab_4
  public  :: pme_recip_opt_slab_6
  public  :: pme_recip_opt_slab_8

contains

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    setup_pme_opt_slab
  ...
  subroutine setup_pme_opt_slab(domain, boundary, enefunc, calc)
  ...
  end subroutine setup_pme_opt_slab

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    dealloc_pme_opt_slab
  ...
  subroutine dealloc_pme_opt_slab
    ...
    deallocate(qdf, qdf_real, buf_send, buf_recv, gx, gy, gz, gy1, vir_fact, &
               vir_fact1, theta, theta1, qdf_work, ftqdf, ftqdf_work,        &
               ftqdf2, ftqdf_work2, b2, stat = dealloc_stat)
    if (dealloc_stat /= 0) call error_msg_dealloc

    return

  end subroutine dealloc_pme_opt_slab

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_pre_opt_slab
  ...
  subroutine pme_pre_opt_slab(domain, boundary)
    ...
    do i = 1, domain%num_cell_local
      do ix = 1, domain%num_atom(i)
        u_self_os = u_self_os + domain%charge(ix,i)*domain%charge(ix,i)
      end do 
    end do

    u_self_os = - u_self_os * el_fact * alpha/sqrt(PI)

    return

  end subroutine pme_pre_opt_slab

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_recip_opt_slab_4
  ...
  subroutine pme_recip_opt_slab_4(domain, force, virial, eelec)
...
!ocl norecurrence(qdf)
  ...
  end subroutine pme_recip_opt_slab_4

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_recip_opt_slab_6
  !> @brief        Calculate PME reciprocal part with domain decomposition
  !! @authors      JJ, NT
  !! @param[in]    domain : domain information
  !! @param[inout] force  : forces of target systems
  ...
  subroutine pme_recip_opt_slab_6(domain, force, virial, eelec)
    ...
    !$omp         bscdy, bscdz, kk, iprocx, iprocy, qtmp, grid, half_grid,     &
    !$omp         start, end, nizx, nizy, iix, iiy, iiz, u_2, u_3, vyyd, vzzd, &
    !$omp         vol_factor, u_4, u_5, v_1, v_4, v_5)
    !
#ifdef OMP
    id = omp_get_thread_num()
#else
...
!ocl norecurrence(qdf)
  ...
  end subroutine pme_recip_opt_slab_6

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    pme_recip_opt_slab_8
  !> @brief        Calculate PME reciprocal part with domain decomposition
  !! @authors      JJ, NT
  !! @param[in]    domain : domain information
  !! @param[inout] force  : forces of target systems
  ...
  subroutine pme_recip_opt_slab_8(domain, force, virial, eelec)
    ...
    !$omp         bscdy, bscdz, kk, iprocx, iprocy, qtmp, grid, half_grid,     &
    !$omp         start, end, nizx, nizy, iix, iiy, iiz, u_2, u_3, vyyd, vzzd, &
    !$omp         vol_factor, u_4, u_5, u_6, u_7, v_1, v_4, v_6, v_7)
    !
#ifdef OMP
    id = omp_get_thread_num()
#else
...
!ocl norecurrence(qdf)
  ...
  end subroutine pme_recip_opt_slab_8
