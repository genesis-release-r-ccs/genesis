...
module sp_energy_14_notable_mod
  ...
  private :: compute_energy_nonbond14_notable_gro_amber_generic_fep
  private :: compute_energy_nonbond14_notable_gro_amber_check_fep
...
contains
  ...
  subroutine compute_energy_nonbond14_notable_fep(domain, enefunc, atmcls, &
    ...
    if (enefunc%nonb_limiter) then
      ...
      ! FEP
      call compute_energy_nonbond14_notable_gro_amber_check_fep( &
                              domain, enefunc, coord, force, virial, &
                              eelec, evdw)
      ...
      ! FEP
      call compute_energy_nonbond14_notable_gro_amber_generic_fep( &
                            domain, enefunc, coord, force, virial, &
                            eelec, evdw)
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    compute_energy_nonbond14_notable_gro_amber_generic_fep
  !> @brief        calculate nonbonded14 energy with PME and lookup table
  !!               (linear) for FEP 
  !  @authors      HO
  !! @param[in]    domain  : domain information
  ...
  subroutine compute_energy_nonbond14_notable_gro_amber_generic_fep(domain, &
                                  enefunc, coord, force, virial, eelec, evdw)
    ...
    !$omp         work, table, lj12, lj6, qq_scale, lj_scale, cc, inv_r12,     &
    !$omp         inv_r121, inv_r123, inv_r126, inv_r1212, iatmcls, jatmcls,   &
    !$omp         viri, evdw_temp, elec_temp, fg1, fg2,                        &
    !$omp         rij2_sclj, rij2_scel, trans_x, trans_y, trans_z)
    !
#ifdef OMP
    id = omp_get_thread_num()
#else
  ...
  end subroutine compute_energy_nonbond14_notable_gro_amber_generic_fep

  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    compute_energy_nonbond14_notable_gro_amber_check_fep
  !> @brief        calculate nonbonded14 energy without lookup table of vdw
  !!               (linear) for FEP
  !  @authors      HO
  !! @param[in]    domain  : domain information
  ...
  subroutine compute_energy_nonbond14_notable_gro_amber_check_fep(domain, &
      enefunc, coord, force, virial, eelec, evdw)
    ...
    !$omp         work, table, lj12, lj6, qq_scale, lj_scale, cc, inv_r12,     &
    !$omp         inv_r121, inv_r123, inv_r126, inv_r1212, iatmcls, jatmcls,   &
    !$omp         viri, evdw_temp, elec_temp, trans_x, trans_y, trans_z,       &
    ...
    do ij = id+1, ncell_local, nthread
      ...
      do k = 1, num_nb14_calc(ij)
        ...
        ! FEP: elec with soft core
        rij2  = cutoff2*density / (rij2 + table_scel(fg1,fg2))
        L     = int(rij2)
        R     = rij2 - L
        term_elec   = table_ene(L) + R*(table_ene(L+1)-table_ene(L))
        elec_temp   = elec_temp  + cc*term_elec
  ...
  end subroutine compute_energy_nonbond14_notable_gro_amber_check_fep
